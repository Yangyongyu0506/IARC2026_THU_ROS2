"""transformations: Rotation matrix, quaternion, Euler-angle conversions and plane-fitting helpers."""
...
def rotation_matrix_to_quaternion(R) -> np.ndarray:
    ...
    if trace > 0:
        ...
        if R[0, 0] > R[1, 1] and R[0, 0] > R[2, 2]:
            ...
            q.append(0.25 * s)
    q = np.array(q)
    return np.roll(q, -1)  # (x, y, z, w) order
...
def get_xyz_from_points(points) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    assert points.shape[0] >= 3, (
        "At least 3 points are required to define the coordinate system."
    )
    x0 = points[1] - points[0]
    y0 = points[-1] - points[0]
    x0 /= np.linalg.norm(x0)
    z = get_normal_vector(points)
    z *= np.sign(np.dot(z, np.cross(x0, y0)))
    z /= np.linalg.norm(z)
    y = np.cross(z, x0)
    y /= np.linalg.norm(y)
    x = np.cross(y, z)
...
def force_orthogonal(R) -> np.ndarray:
    """
    Force a rotation matrix to be orthogonal using Singular Value Decomposition (SVD).
    """
